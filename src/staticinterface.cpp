...
#define MAXPIPES		8		//the maximum number of ingredients in the pipes
#define DRINKSIZE		200		//the maximum size of the drink in ml
#define DELAY			3		//the delay after ordering a drink
...
#define LETTERS			12		//display the drinks for that letters from drinks
#define ALLLETTERS		13		//display the drinks for that letters from alldrinks
...
// define the structure for the drink information to send to the machine
struct drinkOrder {
...
struct letter alphabetInfo[26];					// an array of info for the letters of the alphabet
...
int selectMiddleDrink = -1;	// the button refering to order the middle drink
int editMiddleDrink = -1;	// the button refering to edit the middle drink
int cancelMiddleDrink = -1;	// the button refering to cancel the middle drink
...
if (dynamic == 1){
	if (state != INTRO && state != ORDER){
		...
		displayText(-dynSize + 0.5,dynSize + 0.3,0.0,1.0,1.0,1.0,"Drinks Related to ",1.0);
		if (strcmp(movingButtons[centerballBalls].text,"null") != 0){
			displayText(-dynSize+3.1,dynSize + 0.3,0.0,1.0,1.0,1.0,movingButtons[centerballBalls].text,1.0);
		}
		if (dynButton == 1){
			...
			if (i == now){
				for (m=0;m<5;m++){
					displayText(xi + movingButtons[i].size,
						yi-(1+m)*movingButtons[i].size,0.0,1.0,1.0,1.0,
...
void action(int ball)
{
	...
	case DRINKS:
		 if (ball == back){
			...
			if (dynLastButton == 0)
				numButtons = numButtons - 2*numLetters;
		}
		break;
	case ALLDRINKS:
		 if (ball == back){
	...
	case ORDER:
		if (ball == next){	// order confirmed, sent messages
			...
			openingBalls();
			state = INTRO;
		} if (ball == next2){	// order confirmed, sent messages
			setLastDrink();
			initBalls();
			display();
			placeOrder(2);
